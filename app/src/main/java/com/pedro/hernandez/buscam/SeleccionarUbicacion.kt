...
import com.pedro.hernandez.buscam.databinding.ActivitySeleccionarUbicacionBinding
...
class SeleccionarUbicacion : AppCompatActivity(), OnMapReadyCallback {
    ...
    private fun direccionLating(latlng: LatLng) {
        val geocoder = Geocoder(this)
        try {
            val adressList = geocoder.getFromLocation(latlng.latitude, latlng.longitude, 1)
            val address = adressList!![0]
            val adressLine = address.getAddressLine(0)
            val subLocality = address.subLocality
            direction = "$adressLine"
            agregarMarcador(latlng, "$subLocality", "$adressLine")
        }catch (e:Exception){
